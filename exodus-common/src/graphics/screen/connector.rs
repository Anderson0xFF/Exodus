...
use crate::{debug, enums::*, graphics::{device::GPUID, encoders::Encoder}, error};
...
impl Connector {
    pub(crate) fn new(gpu: GPUID, connector_id: u32) -> Result<Option<Self>, ErrorKind> 
    {
        ...
        let mode = modes[0];
        debug!("Default mode selected: {:?}", unsafe {mode.as_ref().unwrap()});
    ...
    unsafe fn get_connector(device: GPUID, connector_id: u32) -> Result<Option<*mut _drmModeConnector>, ErrorKind> 
    {
        debug!("Getting connector. - GPUID: {}, ConnectorID: {}", device, connector_id);
